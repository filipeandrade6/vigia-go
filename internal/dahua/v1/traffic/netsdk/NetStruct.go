...
type EM_RACE_TYPE int32 // 
const (
	EM_RACE_UNKNOWN EM_RACE_TYPE = iota // 
	EM_RACE_NODISTI                     // 
	EM_RACE_YELLOW                      // 
	EM_RACE_BLACK                       // 
	EM_RACE_WHITE                       // 
...
type SDK_PIC_INFO struct {
	...
	ST_bReserved    [2]BYTE   // 预留字节数
	ST_byQulityScore	BYTE  // 人脸抓拍质量分数, 0-100
...
type DEV_EVENT_TRAFFICJUNCTION_INFO struct {
	...
	ST_stuPlateInfo		  EVENT_PLATE_INFO					// 车辆信息，记录了车头、车尾车牌号和车牌颜色
	ST_byReserved2        [1652]BYTE                        // 保留字节,留待扩展
}
...
type NET_FACE_ATTRIBUTE_EX struct {
	...
	ST_byReserved2       [4]BYTE      		// 
	ST_emStrabismus   EM_STRABISMUS_TYPE  // 斜视状态
	ST_stuObjCenter		NET_POINT			// 物体型心(不是包围盒中心), 0-8191相对坐标, 相对于大图
	ST_byReserved     [60]BYTE            // 保留字节,留待扩展.
}
